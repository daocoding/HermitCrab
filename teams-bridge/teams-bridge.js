...
// ClaudeBridge — SDK-based ACP agent lifecycle manager (v1)
...
function initClaudeBridge() {
  ...
  try {
    if (!ClaudeBridge) {
      ClaudeBridge = require("../lib/claude-bridge").ClaudeBridge;
    }
    ...
    claudeBridge.on("turn_complete", (data) => {
      log("CB_TURN_COMPLETE", { sessionKey: data.sessionKey, status: data.status, chars: data.text?.length || 0, cost: data.costUsd || 0 });
    });
    claudeBridge.on("session_failed", (data) => {
      log("CB_SESSION_FAILED", { sessionKey: data.sessionKey, error: data.error, retries: data.retries });
    });
    console.log("[BRIDGE] ClaudeBridge initialized (SDK in-process)");
  } catch (e) {
...
const PATH_ENV = `${path.dirname(CLI_PATH)}:/opt/homebrew/bin:/usr/local/bin:${process.env.PATH || "/usr/bin:/bin"}`;
...
const AGENTS = {
  ...
  cody: {
    ...
    engine: "claude", // Uses ClaudeBridge (SDK in-process) via useBridge flag
    workspace: path.join(WORKSPACE, "hermitcrab", "cody-workspace"), // CLAUDE.md lives here
    useBridge: true, // Route through ClaudeBridge (SDK-based ACP)
    model: "sonnet", // Use sonnet to conserve Opus quota (shared with Big)
  },
};
...
async function drainCollect(agentKey, conversationId, queuedMessages, serviceUrl) {
  ...
  if (agent?.useBridge && initClaudeBridge()) {
    await wakeAgentViaBridge(agentKey, conversationId, senderName, collectPrompt, serviceUrl);
  } else if (agent?.engine === "claude") {
...
// ═══════════════════════════════════════════
// WAKE AGENT VIA CLAUDE BRIDGE — SDK-based ACP session path
// Used for agents with useBridge: true (currently Cody).
// Uses @anthropic-ai/claude-agent-sdk in-process with session persistence.
// Bridge captures response text and posts via Graph API.
// ═══════════════════════════════════════════
async function wakeAgentViaBridge(agentKey, conversationId, senderName, text, serviceUrl) {
  ...
  // Build the user message (same format as wakeAgentClaude)
  const historyBlock = getRecentHistory(conversationId);
  const session = initClaudeBridge().getSession(sKey);
  const isNewSession = !session.sessionId;
  const fullPrompt = isNewSession && historyBlock
    ? `${historyBlock}\nNew message from ${senderName}: "${text}"`
    : text;

  // Build system prompt (same as wakeAgentClaude)
  const systemPrompt = [
    agent.personaText || "",
  ...
  try {
    const result = await initClaudeBridge().runTurn(sKey, fullPrompt, {
    ...
    if (result.status === "completed" && result.text) {
      ...
      try {
        ...
        log("CB_GRAPH_ERROR", { error: graphError.message, response: result.text.substring(0, 200) });
        // Fallback: try Bot Framework response
        try {
          await sendToTeams(serviceUrl, conversationId, `**${agent.name}**: ${result.text}`);
        } catch (_) { }
      }
      ...
      log("CB_CANCELLED", { agent: agentKey, session_key: sKey });
    } else {
      log("CB_EMPTY", { agent: agentKey, session_key: sKey, status: result.status });
    }
  } catch (err) {
    ...
    disarmWatchdog(sKey);
    sealTyping(conversationId);
    const queuedSession = busySessions.get(sKey);
    const queued = queuedSession?.queue || [];
    busySessions.delete(sKey);
    log("BUSY_CLEAR", { agent: agentKey, conversation_id: conversationId, session_key: sKey });

    // Drain queue if messages accumulated while busy
    if (queued.length > 0) {
      const combined = queued.map(q => q.fullMessage).join("\n");
      log("CB_QUEUE_DRAIN", { agent: agentKey, queued: queued.length });
      wakeAgentViaBridge(agentKey, conversationId, queued[queued.length - 1].senderName, combined, serviceUrl).catch(e => {
        log("CB_QUEUE_DRAIN_ERROR", { error: e.message });
      });
    }
  }
}
          ...
          if (agentSession?.busy) {
            ...
            if (isGroupChat) {
              ...
              if (existingUUID) {
                ...
                if (agnt?.useBridge && initClaudeBridge()) {
                  await wakeAgentViaBridge(agentKey, conversationId, primarySender, combinedMessage, primaryServiceUrl);
                } else if (agnt?.engine === "claude") {
...
const replyServer = http.createServer(async (req, res) => {
  ...
  // Debug probe: test ClaudeBridge from inside the bridge process
  if (req.method === "GET" && req.url === "/sdk-probe") {
    res.writeHead(200, { "Content-Type": "application/json" });
    (async () => {
      try {
        // Direct claude --print test from bridge process
        let claudeResult = "unknown";
        try {
          claudeResult = require("child_process").execSync(
            'PATH=/opt/homebrew/bin:$PATH claude --print --model sonnet --output-format json -- "pong" 2>&1',
            { encoding: "utf-8", timeout: 30000, cwd: WORKSPACE }
          ).substring(0, 500);
        } catch (e) { claudeResult = "error: " + (e.stdout || e.stderr || e.message).substring(0, 500); }
        res.end(JSON.stringify({ ok: true, claudeResult }));
        return;
        res.end(JSON.stringify({ ok: true, status: result.status, text: result.text?.substring(0, 50), session_id: result.sessionId }));
      } catch (e) {
        res.end(JSON.stringify({ ok: false, error: e.message.substring(0, 500) }));
      }
    })();
    return;
  }
...
async function performGracefulRestart(signal) {
  ...
  if (claudeBridge) {
    log("SYSTEM", { event: "claude_bridge_shutdown", sessions: claudeBridge.sessions.size });
